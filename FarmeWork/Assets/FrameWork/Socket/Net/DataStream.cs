...
namespace ShipFarmeWork.Net
{
    ...
    public class DataStream
    {
        ...
        private bool mBEMode;    //硬件是否是大端模式

        public DataStream()
        {
            mMemStream = new MemoryStream();
            InitWithMemoryStream(mMemStream);
        }
        ...
        public DataStream(byte[] buffer, int index, int count)
        {
            mMemStream = new MemoryStream(buffer, index, count);
            InitWithMemoryStream(mMemStream);
        }

        private void InitWithMemoryStream(MemoryStream ms)
        {
            mBinReader = new BinaryReader(mMemStream);
            mBinWriter = new BinaryWriter(mMemStream);
            mBEMode = !BitConverter.IsLittleEndian;
        }
        ...
        //新加读取剩余字节的逻辑,从当前读取的位置往后读取
        public byte[] ReadRemainBytes()
        {
            int len = (int)(mMemStream.Length - mMemStream.Position);
            return mBinReader.ReadBytes(len);
        }
